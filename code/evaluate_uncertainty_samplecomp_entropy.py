...
from uncertainty_methods_miniVGG import minivgg_deepensemble as unc_deepensemble
from uncertainty_methods_miniVGG import minivgg_subensemble as unc_subeensemble
from uncertainty_methods_miniVGG import minivgg_dropout as unc_dropout
from uncertainty_methods_miniVGG import minivgg_dropconnect as unc_dropconnect
from uncertainty_methods_miniVGG import minivgg_duq as unc_duq
from uncertainty_methods_miniVGG import minivgg_bayesbackprop as unc_bayesbackprop
from uncertainty_methods_miniVGG import minivgg_flipout as unc_flipout
from uncertainty_methods_miniVGG import minivgg_baseline as unc_baseline

UNCERT_METHODS = {
    "deepensemble": unc_deepensemble,
    "subensemble": unc_subeensemble,
    "dropconnect": unc_dropconnect,
    "dropout": unc_dropout,
    "duq": unc_duq,
    "baseline": unc_baseline,
    "bayesbackprop": unc_bayesbackprop,
    "flipout": unc_flipout
}
...
if __name__ == "__main__":
    ...
    parser.add_argument("uncertainty_method", help="Name of the uncertainty method to use")

    args = parser.parse_args()
    dataset = args.dataset
    uncert_method = args.uncertainty_method
    uncert_method_fn = UNCERT_METHODS[uncert_method]
    ...
    output_csv_filename = 'entropy-vs-SPC-{}-results-{}-{}.csv'.format(dataset, model_name, uncert_method)
    ...
    results = pd.DataFrame(columns = ['spc', 'acc', 'mean_entropy', 'std_entropy', 'median_entropy', 'uq_entropy', 'lq_entropy', 'uw_entropy', 'lw_entropy',
                                                    'mean_maxprob', 'std_maxprob', 'median_maxprob', 'uq_maxprob', 'lq_maxprob', 'uw_maxprob', 'lw_maxprob',
                                                    'ece', 'ood_auc_entropy', 'ood_auc_maxprob', 'brier', 'train_ece',
                                                    'tr_test_auc_entropy', 'tr_test_auc_maxprob', 'tr_ood_auc_entropy', 'tr_ood_auc_maxprob'])
    ...
    for spc in SAMPLES_PER_CLASS:
        ...
        for i in range(TRIALS):
            ...
            model = uncert_method_fn(input_shape, (x_sample, y_sample), epochs, batch_size)                    
            ...
            entropy = entropy.clip(min=0.0)
            maxprob = np.max(preds, axis=1)
            ...
            uq_entropy = np.quantile(entropy, q=0.75)
            lq_entropy = np.quantile(entropy, q=0.25)
            
            uw_entropy = upper_whisker(entropy)
            lw_entropy = lower_whisker(entropy)
            ...
            brier = mean_squared_error(y_test, preds)
            brier = round(brier, 3)

            # Calibration error
            class_conf = np.max(preds, axis=1)
            ...
            train_class_conf = np.max(train_preds, axis=1)
            ...
            ood_score_entropy = numpy_entropy(ood_preds)
            ood_score_maxprob = 1.0 - np.max(ood_preds, axis=1)
            auc_entropy = roc_auc_score(y_ood, ood_score_entropy)
            auc_maxprob = roc_auc_score(y_ood, ood_score_maxprob)

            train_entropy = numpy_entropy(train_preds)
            test_entropy = ood_score_entropy[:x_test.shape[0]]
            tr_test_entropy = np.concatenate([train_entropy, test_entropy])

            train_maxprob = np.max(train_preds, axis=1)
            test_maxprob = np.max(ood_preds[:x_test.shape[0]], axis=1)
            tr_test_maxprob = np.concatenate([train_maxprob, test_maxprob])

            tr_test_ood_y = np.concatenate([np.zeros(train_preds.shape[0]), np.ones(x_test.shape[0])])
            tr_test_auc_entropy = roc_auc_score(tr_test_ood_y, tr_test_entropy)
            tr_test_auc_maxprob = roc_auc_score(tr_test_ood_y, 1.0 - tr_test_maxprob)

            ood_entropy = ood_score_entropy[x_test.shape[0]:]
            tr_ood_entropy = np.concatenate([train_entropy, ood_entropy])
            tr_ood_maxprob = np.concatenate([1.0 - train_maxprob, ood_score_maxprob[x_test.shape[0]:]])
            tr_ood_y = np.concatenate([np.zeros(train_entropy.shape[0]), np.ones(ood_entropy.shape[0])])
            tr_ood_auc_entropy = roc_auc_score(tr_ood_y, tr_ood_entropy)
            ...
            auc_entropy = round(auc_entropy, 3)
            auc_maxprob = round(auc_maxprob, 3)

            print("SPC {} Test Accuracy: {:.3f} Entropy {:.3f} +- {:.3f} ECE {:.3f} AUC EN {:.3f} AUC MP {:.3f}".format(spc, acc, mean_entropy, std_entropy, cab_err, auc_entropy, auc_maxprob))
            
            results_dict = {'spc': spc, 'acc': acc, 'mean_entropy': mean_entropy, 'std_entropy': std_entropy, 'mean_maxprob': mean_maxprob, 'std_maxprob': std_maxprob,
                            'ece': cab_err, 'ood_auc_entropy': auc_entropy, 'ood_auc_maxprob': auc_maxprob, 'brier': brier}

            results_dict['median_entropy'] = median_entropy
            results_dict['uq_entropy'] = uq_entropy
            results_dict['lq_entropy'] = lq_entropy
            results_dict['uw_entropy'] = uw_entropy
            results_dict['lw_entropy'] = lw_entropy
            ...
            results_dict['tr_test_auc_entropy'] = tr_test_auc_entropy
            results_dict['tr_test_auc_maxprob'] = tr_test_auc_maxprob
            results_dict['tr_ood_auc_entropy'] = tr_ood_auc_entropy
